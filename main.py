...
async def print_message(title, msg):
        ...
        if title == 'Orderbook' :
                ...
                exists_bid = False
                exists_ask = False
                if len(decoded_msg['bidDeltas']) > 0 : 
                    bidAmount =  float(decoded_msg['bidDeltas'][0]['quantity'])
                    bidRate =    float(decoded_msg['bidDeltas'][0]['rate'])
                    exists_bid = True
                if len(decoded_msg['askDeltas'])   > 0 : 
                  askAmount = float(decoded_msg['askDeltas'][0]['quantity'])
                  askRate = float(decoded_msg['askDeltas'][0]['rate'])
                  exists_ask = True
                
                if decoded_msg['marketSymbol'] not in engine.initial_df_with_all_tickers.index : 
                  buffer_df = pd.DataFrame(columns = engine.initial_df_with_all_tickers.columns )
                  buffer_df.loc[0] =   [ askAmount , askRate  , bidAmount , bidRate ] 
                  buffer_df.index  =  [decoded_msg['marketSymbol']]  
                  engine.initial_df_with_all_tickers = engine.initial_df_with_all_tickers.append(buffer_df)
                  return None 
                
                 
                if exists_bid: 
                  engine.initial_df_with_all_tickers.at[decoded_msg['marketSymbol']  ,'BidRate' ]  = bidRate
                  engine.initial_df_with_all_tickers.at[decoded_msg['marketSymbol']  , 'BidAmount' ]  = bidAmount
                if exists_ask: 
                  engine.initial_df_with_all_tickers.at[decoded_msg['marketSymbol']  ,'AskAmount' ] = askAmount
                  engine.initial_df_with_all_tickers.at[decoded_msg['marketSymbol']  , 'AskRate' ]  = askRate
...
def detect_and_full_fill_arbitrage() :
  engine.get_data_from_the_whole_orderbook()
      
def get_Data_from_the_orderbook():
  asyncio.run(main())    
...
if __name__ == "__main__":
  t1 = threading.Thread(target = detect_and_full_fill_arbitrage)
  t2 = threading.Thread(target =get_Data_from_the_orderbook)
